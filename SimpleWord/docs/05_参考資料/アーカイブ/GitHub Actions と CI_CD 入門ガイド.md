...
### 背景
iOS 17で`onChange`構文が非推奨になり、今後も同様の問題を防ぐため、5層の防御システムを構築。
AIモデルが自動的に非推奨構文を回避し、コミット前・CI/CD時に自動検出する仕組みを実装。
...
#### 5. ドキュメント層（教育・参照）
- **`DEPRECATION_GUIDE.md`** (新規作成)
  - セットアップと使用方法
  
- **`参考資料/GitHub Actions と CI_CD 入門ガイド.md`** (新規作成)
  - CI/CDの基礎から応用まで
  - 初心者向けの詳細解説

- **`.copilot/anti-deprecation-summary.md`** (新規作成)
  - システム全体のサマリー
