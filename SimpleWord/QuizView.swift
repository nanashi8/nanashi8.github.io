...
└── prompts/                     # プロンプトテンプレート（任意）
    ├── refactor-component.md    # 通常のリファクタリング（300行以下）
    ├── large-scale-refactoring.md  # 大規模リファクタリング（500行以上）
    ├── ai-git-workflow.md       # Git統合ワークフロー（新規）
    ├── version-management.md    # バージョン管理ルール（新規）
    ├── add-feature.md
    └── fix-bug.md
```
...
完了したら Phase 1, Step 1.1 から段階的に進めてください。
各ステップで必ずビルド・テスト・動作確認を行い、Git にコミットしてください。
問題が発生したら即座に中断してロールバックしてください。」
```
