...
1. **パッセージへの影響**: 7,679箇所で単語の意味が表示できない
1. **学習アプリの致命的問題**: 意味がない単語があると学習できない
...
1. 基本的な英単語辞書を拡張して自動マッピング
1. 複数形・過去形・ing形などの活用形を原形から推論
...
1. **第2段階**: 活用形推論（効率的・確実性高）
1. **第3段階**: 文脈推論（時間かかるが重要）
1. **第4段階**: カテゴリ別（追加の効率化）
1. **第5段階**: 手動処理（最後の仕上げ）
...
1. **誤った意味の推論**: 信頼度スコアを付与し、低い場合は手動確認
1. **活用形の誤判定**: 辞書の原形と照合して確認
