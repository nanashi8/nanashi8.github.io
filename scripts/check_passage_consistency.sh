...
# Python版が利用可能ならそちらを使用（存在する場合のみ）
if command -v python3 &> /dev/null && [ -f "$SCRIPT_DIR/check_passage_consistency.py" ]; then
    echo -e "${CYAN}Running Python version for detailed analysis...${NC}\n"
    python3 "$SCRIPT_DIR/check_passage_consistency.py"
    exit $?
fi

echo -e "${YELLOW}Python版が利用できないため、基本的なシェルチェックを実行します...${NC}\n"
