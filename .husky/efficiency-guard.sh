#!/bin/bash
# 効率化ガード - 無駄な作業を検出

set -e

# 色定義
RED='\033[0;31m'
YELLOW='\033[1;33m'
GREEN='\033[0;32m'
NC='\033[0m'

echo "⚡ 効率化ガード実行中..."

# 1. 大量のドキュメント追加を検出
DOCS_ADDED=$(git diff --cached --name-only --diff-filter=A | grep -E '^docs/.*\.md$' | wc -l)
if [ "$DOCS_ADDED" -gt 2 ]; then
  echo -e "${YELLOW}⚠️  警告: ${DOCS_ADDED}個のドキュメントファイルが追加されています${NC}"
  echo "   本当に必要ですか？（個人開発では最小限に）"
  echo ""
fi

# 2. 不要なREADME以外のマークダウンファイル追加を検出
SUMMARY_FILES=$(git diff --cached --name-only --diff-filter=A | grep -E '(SUMMARY|REPORT|COMPLETE).*\.md$' | wc -l)
if [ "$SUMMARY_FILES" -gt 0 ]; then
  echo -e "${YELLOW}⚠️  警告: サマリー/レポートファイルが追加されています${NC}"
  echo "   効率化ガード: このファイルは本当に必要ですか？"
  echo ""
fi

# 3. feature/ブランチが残っているか確認
CURRENT_BRANCH=$(git rev-parse --abbrev-ref HEAD)
if [[ "$CURRENT_BRANCH" == feature/* ]]; then
  echo -e "${YELLOW}⚠️  警告: feature/ブランチにいます${NC}"
  echo "   個人開発では直接mainにマージを推奨"
  echo "   git checkout main && git merge $CURRENT_BRANCH"
  echo ""
fi

# 4. .aitk/.commit-countをチェック（1回のタスクで5コミット以上は多い）
if [ -f ".aitk/.commit-count" ]; then
  COMMIT_COUNT=$(cat .aitk/.commit-count)
  if [ "$COMMIT_COUNT" -gt 5 ]; then
    echo -e "${YELLOW}⚠️  警告: コミット数が${COMMIT_COUNT}回です${NC}"
    echo "   タスクを細分化しすぎている可能性があります"
    echo ""
  fi
fi

echo -e "${GREEN}✅ 効率化ガードチェック完了${NC}"
exit 0
