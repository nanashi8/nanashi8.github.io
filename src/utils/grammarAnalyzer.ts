...
export type GrammarTag = 'S' | 'V' | 'O' | 'C' | 'M' | 'Prep' | 'Conj' | 'Det' | 'Adj' | 'Adv' | 'Unknown';
...
const PREPOSITIONS = new Set([
  ...
  'during', 'without', 'before', 'under', 'around', 'among', 'of', 'up'
]);
...
const ADJECTIVES = new Set([
  ...
  'able', 'ready', 'usual', 'toast', 'juice'
]);

const ADVERBS = new Set([
  'not', 'so', 'up', 'out', 'just', 'now', 'how', 'then', 'more',
  'also', 'here', 'well', 'only', 'very', 'even', 'back', 'there',
  'down', 'still', 'in', 'as', 'too', 'when', 'never', 'really',
  'usually', 'finally', 'first', 'after', 'before'
]);
...
const GRAMMAR_PATTERNS: GrammarPattern[] = [
  ...
  },
  {
    ...
    explanation: '「It is + 形容詞 + for + 人 + to + 動詞」の形で、「(人)が〜するのは...だ」という意味'
  },
  {
    name: 'It is ~ to ...',
    meaning: '〜することは...だ',
