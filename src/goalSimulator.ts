...
export function calculateGoalProgress(goal?: GoalLevel): GoalProgress {
  ...
  // 達成判定
  const isAchieved = currentWords >= targetGoal.requiredWords && 
                     currentAccuracy >= targetGoal.requiredAccuracy;
  ...
  const recommendedDailyWords = estimatedDaysToAchieve > 0 
    ? Math.ceil(remainingWords / estimatedDaysToAchieve)
    : 0;
...
export function generateGoalMessage(includeEncouragement: boolean = true): string {
  ...
  // 進捗メッセージ
  messages.push(`${goalProgress.goal.icon} ${goalProgress.goal.name}まで ${goalProgress.overallProgress}%`);
...
export function suggestRealisticGoal(): GoalLevel {
  ...
  // 最適な目標を選択
  const suitableGoals = GOAL_LEVELS.filter(g => 
