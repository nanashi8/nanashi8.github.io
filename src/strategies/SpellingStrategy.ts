 ...
 * - 正答率ベースの優先度
 * - タイピング速度の考慮（将来拡張用）
 * - 長単語・特殊文字の優先度調整（将来拡張用）
 * - 効果的上限の適用
 */
export class SpellingStrategy extends BaseQuestionStrategy<Question> {
  constructor() {
    super('spelling');
  }
  ...
  sortQuestions(questions: Question[], limits: LearningLimits, stats?: SessionStats): Question[] {
    ...
    try {
      ...
      // ソート: 優先度 > 単語の長さ（長い方を優先） > 最終学習時刻（古い順） > ランダム
      const sorted = questionsWithStatus.sort((a, b) => {
        ...
        // 単語の長さ（長い方がタイピング練習に有効）
        const lengthA = a.question.word.length;
        const lengthB = b.question.word.length;
        if (Math.abs(lengthA - lengthB) >= 3) {
          // 3文字以上差がある場合のみ考慮
          return lengthB - lengthA; // 長い方を優先
        }
      ...
      logger.error('SpellingStrategy: ソート中にエラーが発生しました:', error);
      return questions;
    }
  }
}
