...
export class PersonalParameterEstimator {
  ...
  private estimateConsolidationThreshold(history: LearningHistory[]): number {
    ...
    const averageCount = consolidationCounts.reduce((sum, c) => sum + c, 0) / consolidationCounts.length;
  ...
  private estimateResponseTimeProfile(history: LearningHistory[]): ResponseTimeProfile {
    ...
    // 標準偏差
    const variance = responseTimes.reduce((sum, t) => sum + Math.pow(t - averageResponseTime, 2), 0) / responseTimes.length;
