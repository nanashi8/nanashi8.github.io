 ...
 * 
 * 特徴:
 * - 正答率ベースの優先度（50%未満=incorrect, 50-80%=learning, 80%以上=mastered）
 * - 学習履歴の考慮（最終学習日時の古い順）
 * - シンプルなソートロジック（高速）
 * - 効果的上限の適用
 */
export class TranslationStrategy extends BaseQuestionStrategy<Question> {
  constructor() {
    super('translation');
  }
  ...
  sortQuestions(
    ...
    try {
      ...
      logger.error('TranslationStrategy: ソート中にエラーが発生しました:', error);
      return questions;
    }
  }
}
