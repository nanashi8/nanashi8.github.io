...
export function buildUserProfile(
  ...
  // 学習ペースの計算
  const now = Date.now();
  const recentProgress = wordProgresses.filter(wp => 
    now - wp.lastStudied < 7 * 24 * 60 * 60 * 1000 // 過去7日間
  );
  
  const dailyAverage = recentProgress.length > 0 
  ...
  // 動的閾値の設定
  const overallAccuracy = (difficultyMastery.beginner + difficultyMastery.intermediate + difficultyMastery.advanced) / 3;
...
// 単語の期待正答率を計算
export function getExpectedAccuracy(
  question: Question,
...
export function calculatePriority(
  ...
  // 習得に近い単語を優先(正答率50-79%)
  if (wordProgress) {
    const accuracy = wordProgress.correctCount > 0 
      ? (wordProgress.correctCount / (wordProgress.correctCount + wordProgress.incorrectCount)) * 100 
      : 0;
  ...
  const actual = wordProgress 
    ? (wordProgress.correctCount / Math.max(1, wordProgress.correctCount + wordProgress.incorrectCount)) * 100 
    : 50;
