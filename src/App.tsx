...
if (import.meta.env.DEV) {
  import('./utils/debugAIEvaluations');
}
...
function App() {
      ...
      try {
        ...
        if (allQuestions.length > 0) {
          ...
          const customQuestionSets: QuestionSet[] = customSets.map((cs) => {
            ...
            const sourcePrefix =
              cs.source === 'reading' ? '長文' : cs.source === 'weak-words' ? '苦手語句' : '手動';
  ...
  const handleStartQuiz = async () => {
    ...
    if (improvementProgress) {
      ...
      logger.log(
        `📊 改善進捗: ${improvementProgress.currentDay}日目 - 全体進捗${improvementProgress.overallProgress.toFixed(1)}%`
      );
    ...
    if (radarAnalysis.weakCategories.length > 0 && selectedCategory === 'all') {
      ...
      logger.log(
        `💡 弱点分野を優先出題: ${radarAnalysis.weakCategories
          .slice(0, 3)
          .map((w) => w.category)
          .join(', ')}`
      );
    ...
    */
    ...
    if (todayIncorrect.length > 0) {
      ...
      if (reviewQuestions.length > 0) {
        logger.log(
          `🔄 要復習問題: ${reviewQuestions.length}問（上限: ${studySettings.maxReviewCount}問）`
        );
      }
    }
    ...
    if (!reviewFocusMode && filteredQuestions.length > 0) {
      ...
      logger.log(
        ...
        adjustedSequence
          .slice(0, 5)
          .map((p) => `${p.word}(${p.strategy}, 成功率${p.estimatedSuccessRate.toFixed(0)}%)`)
          .join(', ')
      );
    ...
    if (highRisk.length > 0) {
      ...
      logger.log(
        `  最高リスク: ${highRisk[0].word} (${highRisk[0].errorRisk.toFixed(0)}% - ${highRisk[0].primaryPattern})`
      );
    }
  };
    ...
    if (currentQuestion) {
      ...
      if (skipWordProgress && skipWordProgress.skippedCount && skipWordProgress.skippedCount > 0) {
        ...
        if (isCorrect) {
          ...
          logger.log(
            '🤔 AI学習アシスタント: スキップした単語が不正解でした。同時期の単語を再確認します。'
          );
        }
      }
