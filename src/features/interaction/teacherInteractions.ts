  ...
  if (mainTeacher === 'drill-sergeant') {
    if (isCorrect && streak >= 5) {
      patterns.push({
        ...
        message:
          '😈「よし！その調子だ！」 😃「ちょ、ちょっと褒めすぎでは...」 😈「うるさい！良いものは良いんだ！」',
        type: 'banter',
      });
      patterns.push({
        mainTeacher: 'drill-sergeant',
    ...
    if (!isCorrect) {
      patterns.push({
        ...
        otherTeacher: 'kind-teacher',
        message:
          '😈「何度同じ間違いを...！」 😃「まあまあ、落ち着いて。誰にでも苦手はありますから」 😈「甘やかすな！」',
        type: 'argument',
      });
      patterns.push({
        mainTeacher: 'drill-sergeant',
        ...
        message:
          '😈「たるんでるぞ！」 🤖「統計的には許容範囲内のミスです」 😈「データじゃない、気持ちの問題だ！」',
        type: 'argument',
      });
    }
  ...
  if (mainTeacher === 'kind-teacher') {
    if (isCorrect) {
      patterns.push({
        ...
        message:
          '😃「素晴らしいですね！」 😈（遠くから）「まだまだ油断するな！」 😃「...聞こえてます？」',
        type: 'passerby',
      });
      patterns.push({
        mainTeacher: 'kind-teacher',
        otherTeacher: 'wise-sage',
        message:
          '😃「よくできました！」 🧙「うむ、着実な成長じゃ」 😃「賢者先生もそう思われますよね」',
        type: 'support',
      });
    }
    ...
    if (!isCorrect) {
      patterns.push({
        ...
        otherTeacher: 'enthusiastic-coach',
        message:
          '😃「大丈夫、次は必ずできますよ」 😼「そうそう！失敗は成功の母だ！」 😃「熱血コーチも応援してますよ」',
        type: 'support',
      });
    }
    ...
    patterns.push({
      mainTeacher: 'kind-teacher',
      message:
        '😃「あ、お茶を入れ忘れて...」 😈（遠くから）「仕事中に何してる！」 😃「す、すみません！」',
      type: 'joke',
    });
  }
  ...
  if (mainTeacher === 'analyst') {
    if (streak >= 10) {
      patterns.push({
        ...
        otherTeacher: 'enthusiastic-coach',
        message:
          '🤖「10連続正解。統計的に有意です」 😼「すげー！最高だ！」 🤖「...感情表現は不要です」',
        type: 'banter',
      });
    }
    ...
    patterns.push({
      ...
      otherTeacher: 'drill-sergeant',
      message:
        '🤖「データに基づくと...」 😈「理屈はいい！気合だ！」 🤖「非科学的です」 😈「うるさい！」',
      type: 'argument',
    });
    ...
    patterns.push({
      ...
      otherTeacher: 'kind-teacher',
      message:
        '🤖「正答率73.4%」 😃「あの...もう少し優しく伝えられませんか？」 🤖「事実を述べているだけです」',
      type: 'banter',
    });
    ...
    patterns.push({
      mainTeacher: 'analyst',
      message:
        '🤖「...コーヒーのカフェイン含有量は...」 😃「分析官先生、今は授業中ですよ」 🤖「失礼しました」',
      type: 'joke',
    });
  }
  ...
  if (mainTeacher === 'enthusiastic-coach') {
    if (isCorrect && streak >= 5) {
      ...
      patterns.push({
        ...
        otherTeacher: 'drill-sergeant',
        message:
          '😼「よっしゃー！」 😈「騒がしい...だが、悪くないな」 😼「おっ、鬼軍曹に褒められた！」',
        type: 'support',
      });
    }
  ...
  if (mainTeacher === 'wise-sage') {
    if (isCorrect) {
      patterns.push({
        ...
        otherTeacher: 'drill-sergeant',
        message:
          '🧙「良き成長じゃ」 😈「当たり前だ、俺の教え子だからな」 🧙「...お主も教えておったのか？」',
        type: 'banter',
      });
    }
    ...
    patterns.push({
      ...
      otherTeacher: 'enthusiastic-coach',
      message:
        '🧙「焦らず、じっくりと...」 😼「たまには全力疾走も大事っすよ！」 🧙「...それも一理あるか」',
      type: 'banter',
    });
  ...
  const commonPatterns: TeacherInteraction[] = [
    {
      ...
      message:
        '😃「お昼は何食べます？」 😈「時間の無駄だ」 😼「俺はカツ丼！」 🤖「栄養バランスを考慮すべきです」 🧙「...わしは蕎麦がいいのう」',
      type: 'joke',
    },
    {
      mainTeacher: mainTeacher,
      message:
        '😈「...誰だ、職員室でコーヒーこぼしたのは！」 😃「す、すみません...」 😼「ドンマイ！」 🤖「清掃方法を提示します」',
      type: 'joke',
    },
    {
      mainTeacher: mainTeacher,
      message:
        '🧙「...印刷機が動かんのじゃが」 🤖「IT担当を呼びます」 😃「私が見てみますね」 😈「さっさと直せ！」 😼「みんなで協力だ！」',
      type: 'joke',
    },
    {
      mainTeacher: mainTeacher,
      message:
        '😼「今日の授業どうだった？」 😃「みんな頑張ってましたよ」 😈「まだまだだ」 🤖「データで見ると改善傾向です」 🧙「それぞれの成長があるのう」',
      type: 'banter',
    },
  ];
