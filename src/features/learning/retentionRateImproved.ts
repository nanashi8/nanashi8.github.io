 ...
 * 改善版：詳細な定着率計算
 */
export function calculateDetailedRetentionStats(wordProgressMap: {
 ...
 * 表示用サマリーを生成
 */
export function getRetentionSummary(wordProgressMap: {
  [word: string]: WordProgress;
}): RetentionSummary {
...
if (typeof window !== 'undefined') {
  ...
  (window as any).testRetentionScenario = () => {
    ...
    logger.log(
      '内訳: 🟢' +
        result.masteredPercentage +
        '% 🟡' +
        result.learningPercentage +
        '% 🔴' +
        result.strugglingPercentage +
        '%'
    );
