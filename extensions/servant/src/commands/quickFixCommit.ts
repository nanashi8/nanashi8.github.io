 ...
 * ワンクリックでコミットまで完了
 */
export async function quickFixCommit(
  ...
  try {
    ...
    const allFiles = [...new Set([...stagedFiles.map(f => path.relative(workspaceRoot, f)), ...unstagedFiles])];
    ...
    await execAsync(`git commit -m "${commitMessage.replace(/"/g, '\\"')}"`, { cwd: workspaceRoot });
