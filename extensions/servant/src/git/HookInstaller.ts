 ...
 * HookInstaller
 *
 * Git hooksをインストール・管理するクラス。
 * - pre-commit hookの生成とインストール
 * - 既存hookのバックアップ
 * - hookの削除とリストア
 */
export class HookInstaller {
   ...
   *
   * @param hooksDir - .git/hooksディレクトリのパス
   * @returns インストール成功ならtrue
   */
  ...
  /**
   * pre-commit hookスクリプトを生成
   *
   * @returns hookスクリプトの内容
   */
...
# 新しいIDを優先し、互換のため旧IDも試す
code --command servant.validateBeforeCommit >/dev/null 2>&1 || code --command instructionsValidator.validateBeforeCommit

EXIT_CODE=$?

if [ $EXIT_CODE -ne 0 ]; then
  echo "❌ Servant validation failed!"
  echo "💡 Fix the violations using Quick Fix in VSCode, then try committing again."
  exit 1
fi

echo "✅ Servant validation passed!"
exit 0
`;
  }

  /**
   * commit-msg hookをインストール
   *
   * @param hooksDir - .git/hooksディレクトリのパス
   * @returns インストール成功ならtrue
   */
...
# Conventional Commits形式をチェック（オプション）
# 形式: type(scope): message
# 例: feat(ui): add new button component
...
`;
  ...
  /**
   * hooksをアンインストール（バックアップからリストア）
   *
   * @param hooksDir - .git/hooksディレクトリのパス
   * @param hookName - hookの名前（'pre-commit'または'commit-msg'）
   */
  ...
  /**
   * hookが既にインストールされているかチェック
   *
   * @param hooksDir - .git/hooksディレクトリのパス
   * @param hookName - hookの名前
