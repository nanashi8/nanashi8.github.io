...
test.describe('CSSサイズ比較', () => {
  test('ビルドサイズを比較', async ({ page }) => {
    ...
    console.log('📦 CSSサイズ比較:');
    console.log(`   本番: ${(sizes.production / 1024).toFixed(1)} KB`);
    console.log(`   Beta: ${(sizes.beta / 1024).toFixed(1)} KB`);
    console.log(`   差分: ${((sizes.beta - sizes.production) / 1024).toFixed(1)} KB`);

    // 20%以内の差分に収まること（大きく増えていないこと）
    const ratio = sizes.production > 0 ? sizes.beta / sizes.production : 1;
