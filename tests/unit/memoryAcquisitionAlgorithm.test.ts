...
describe('AcquisitionQueueManager', () => {
  ...
  describe('問題番号ベースのデキュー', () => {
    ...
    it('TC2.4: 複数の復習問題がある場合、優先度順に取得される', () => {
      ...
      if (word1Entry) {
        manager.handleCorrectAnswer(word1Entry.word, word1Entry.queueType, 500, word1Entry.difficulty, word1Entry.category);
      }
  ...
  describe('正答時の処理', () => {
    it('TC3.1: 即時復習で正答すると早期復習キューに昇格', () => {
      ...
      if (immediate) {
        manager.handleCorrectAnswer(immediate.word, immediate.queueType, 800, immediate.difficulty, immediate.category);
      }
  ...
  describe('誤答時の処理', () => {
    it('TC4.1: 即時復習で誤答すると即時復習キューに再追加', () => {
      ...
      const immediate = manager.getNextReviewQuestion();
      if (immediate) {
        manager.handleWrongAnswer(immediate.word, immediate.queueType, 2000, immediate.difficulty, immediate.category);
      }
    ...
    it('TC4.3: 誤答すると難易度が上昇する', () => {
      ...
      if (immediate) {
        manager.handleWrongAnswer(immediate.word, immediate.queueType, 3000, immediate.difficulty, immediate.category);
      }
