...
**作成日**: 2025年10月28日  
**最終更新**: 2025年11月7日  
**対応内容**: CSV種類別に最適化された選択肢カード詳細表示の実装、ファイル配置規約
...
- ログファイル（`*.log`） → `logs/`に配置
- 実装レポート（`*_REPORT.md`等） → `docs/04_レポート/`に配置
- ガイドドキュメント（`*_GUIDE.md`） → `docs/03_開発ガイド/`に配置
...
| ファイル種類 | 配置先 | 例 |
|-------------|--------|-----|
| ガイド・チュートリアル | `docs/03_開発ガイド/` | `TEST_GUIDE.md` |
| 実装レポート・設計書 | `docs/04_レポート/` | `FEATURE_REPORT.md` |
| 仕様書 | `docs/01_仕様書/` | `SPEC.md` |
| 実装ガイド | `docs/02_実装ガイド/` | `IMPLEMENTATION.md` |
| アーカイブ | `docs/99_アーカイブ/` | `README_V2.md` |
| ログファイル | `logs/` (.gitignore) | `*.log` |
| 一時ファイル | `tmp/` (.gitignore) | `*.tmp` |

### 🤖 ファイル作成時の指示

**新規ファイルを作成する場合、必ず適切なフォルダに配置してください。**
...
詳細は`docs/03_開発ガイド/FILE_ORGANIZATION.md`を参照してください。
