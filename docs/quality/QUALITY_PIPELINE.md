...
| チェック | ローカル (pre-commit) | GitHub Actions | 頻度 |
|---------|---------------------|----------------|------|
| TypeScript型チェック | ✅ (5秒) | ✅ | 毎コミット |
| CSS Lint | ✅ (3秒) | ✅ | 毎コミット |
| ビルド | ✅ (2秒) | ✅ | 毎コミット |
| Playwright E2E | ❌ (手動) | ✅ | PR時/プッシュ時 |
| システム健康診断 🆕 | ✅ (10秒) | ✅ | 週次/PR時 |
...
コードベースの技術的負債を定期的に検出し、メンテナンスの必要性を自動通知

### 診断項目

1. **localStorage キー一貫性**
   - 命名規則の統一性チェック
   - 重複キーの検出
...
6. **デバッグコード残留**
   - console.log/error/warn の残留チェック
   - 本番環境への混入防止

7. **型定義重複**
   - interface/typeの重複定義検出
   - types.tsへの集約推奨
...
1. **GitHubイシュー自動作成**
   - ラベル: `health-check`, `maintenance`, `automated`
   - タイトル: 「🏥 定期メンテナンス必要: システム健康診断 (日付)」
   - 本文: 診断結果、優先度、対応チェックリスト
