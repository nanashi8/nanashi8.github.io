...
1. **IPA辞書の限界**: eng_to_ipaで30件の手動修正が必要
1. **事後対応型**: 新規データ追加後にエラーが発覚
1. **スクリプトの乱立**: 各エラータイプ専用スクリプト（6本）
