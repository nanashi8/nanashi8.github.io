...
.button-special { background: #667eeb; }  /* わずかに違う色 */
...
2. CSS変数を`variables.css`に抽出
3. .dark-modeセクションを`dark.css`に分離
4. 共通コンポーネントスタイルを個別ファイル化
5. App.cssから段階的に移行
...
2. 小さいコンポーネントから段階的に移行
3. 共通スタイルはグローバルCSSとして保持
4. 移行完了後、古いCSSファイルを削除
...
- [17-styling.md](./17-styling.md) - スタイリング戦略
...
2. **中期 (1-2ヶ月):** テーマファイル分離 → **-30%追加削減**
3. **長期 (必要時):** CSS Modules導入 → **-60%追加削減**
