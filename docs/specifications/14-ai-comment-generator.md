...
```typescript
export function getTeacherComment(
...
```typescript
// aiCommentGenerator.ts
const PROBABILITY_GOAL_COMMENT = 0.10;  // 目標コメント
const PROBABILITY_CONFUSION_GROUP = 0.05; // 混同グループ
const PROBABILITY_TEACHER = 0.10;  // 先生コメント
...
2. **優先順位**: 混同警告 > 連続正解 > その他の順
3. **パフォーマンス**: 毎回の判定でも処理時間は1ms以下
4. **ストレージ**: コメント履歴は保存しない（メモリ節約）
