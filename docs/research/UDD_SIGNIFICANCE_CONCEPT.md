...
**作成日**: 2026年1月14日  
**ステータス**: 構想（根拠整理・適用ガイド）  
**キーワード**: 未知駆動開発（UDD）, Validate-first, リーク遮断, 原因分解, 監査可能性, 品質ゲート, 母国語コーディング
...
それらを、UDDのコア（Validate-first / 未知分類 / 原因分解 / 監査）として統合し、
「母国語コーディング（自然言語の意図→制約→検証）」とも接続する。
...
### 2-1. 「性能」より「偽性と破綻」を先に潰す（Validate-first）
...
4. **停止条件を固定**：No-Go条件（評価が破綻/原因分解不能/偽性支配）を先に書く
...
## 5. 母国語コーディングとの接続（日本語の強みを安全に使う）

母国語コーディングは、日本語の「高文脈・粒度可変・専門語×日常語の混在」により、
**意図の立ち上げ（探索フェーズ）**を速くできる。

一方で、そのまま実装すると事故るため、
UDDのValidate-first（確認質問→制約→テスト→監査）に接続して安全化する。
