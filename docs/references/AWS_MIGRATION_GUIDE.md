...
1. [移行の概要](#1-移行の概要)
2. [現在のアーキテクチャ vs AWS後のアーキテクチャ](#2-現在のアーキテクチャ-vs-aws後のアーキテクチャ)
3. [移行で追加される機能](#3-移行で追加される機能)
4. [技術スタック選定](#4-技術スタック選定)
5. [AWS構成パターン](#5-aws構成パターン)
6. [コスト試算](#6-コスト試算)
7. [移行ステップ](#7-移行ステップ)
...
**タスク（あなたがAWSコンソールで実施）:**
1. RDSインスタンス作成
2. Lambda関数デプロイ（Serverless Framework使用）
3. API Gateway設定
4. Cognito ユーザープール作成
5. S3 + CloudFront設定
6. 環境変数・シークレット管理（Secrets Manager）
...
2. **ユーザー数が20人を超えたらAWS移行検討**
3. **段階的移行**（いきなり全機能ではなく、認証・同期から）
4. **コスト監視しながら拡張**
