...
1. [取得できる情報](#取得できる情報)
1. [主な実装方法](#主な実装方法)
1. [静的サイト向け実装（推奨）](#静的サイト向け実装推奨)
1. [サーバーログによる取得](#サーバーログによる取得)
1. [サードパーティ解析ツール](#サードパーティ解析ツール)
1. [プライバシー・法的注意点](#プライバシー法的注意点)
1. [導入チェックリスト](#導入チェックリスト)
...
1. **バランス型**: Plausible（軽量、Cookie不要、有料）
1. **完全制御**: 自作トラッキング（Netlify Functions + S3等）

### 自前サーバーの場合
1. Nginx/Apacheのアクセスログ活用
1. GoAccessで可視化
1. 必要に応じてELKやMatomoを追加
