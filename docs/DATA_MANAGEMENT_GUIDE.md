...
- [Vocabulary管理（10分野システム）](#vocabulary管理10分野システム)
- [長文パッセージ管理](#長文パッセージ管理)
- [文法問題管理](#文法問題管理)
- [フレーズ翻訳管理](#フレーズ翻訳管理)
- [品質チェック・レポート](#品質チェックレポート)

---

## 📖 Vocabulary管理（10分野システム）
...
2. **学校・学習** - 教育、学問、読み書き、科目
3. **日常生活** - 家庭、衣食住、金銭、宗教行事
4. **人・社会** - 人間関係、感情、仕事、社会活動、一般動詞・形容詞
5. **自然・環境** - 動植物、天気、地形、季節
6. **食・健康** - 食事、身体、医療、栄養
7. **運動・娯楽** - スポーツ、音楽、芸術、趣味
8. **場所・移動** - 地名、建物、交通、方向
9. **時間・数量** - 時間、日付、数、頻度
10. **科学・技術** - 科学、技術、コンピュータ、実験
...
**動作**:
1. 全対象CSVファイルを自動処理
2. バックアップを自動作成（`.backup_meaning_YYYYMMDD_HHMMSS.csv`）
3. 単語・読み・意味から10分野を判定
4. 分類結果レポートを出力
...
2. **バックアップの保持**: スクリプト実行前に手動バックアップを作成
3. **段階的な修正**: 大量のデータ修正は段階的に実施し、各段階で動作確認
4. **ドキュメント更新**: 分類ロジックを変更した場合、このガイドを更新
5. **テスト環境での検証**: 本番環境に適用する前に、ローカル環境で十分にテスト
...
2. **ビルド前検証**: デプロイ前に必ず`npm run build`で自動検証を実行
3. **バックアップの保持**: スクリプト実行前に手動バックアップ（自動バックアップも生成される）
4. **段階的な修正**: 大量のデータ修正は段階的に実施し、各段階で検証
5. **ドキュメントの更新**: 新しい分類ロジックやスクリプトを追加した場合、このガイドを更新
6. **型安全性の維持**: TypeScriptの型定義を変更した場合、constants.jsonも同期更新
