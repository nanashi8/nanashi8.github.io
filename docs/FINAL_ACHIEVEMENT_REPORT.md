...
1. ✅ **20,000語達成** - 完了!
2. ⏳ 全11パッセージを `reading-passages-comprehensive.json` に統合
3. ⏳ `validate_passages.py` による品質検証

### 優先度: 中

1. ⏳ 未登録語彙203語をCSVに追加
2. ⏳ 語彙レベル違反の最終確認 (目標: ≤10件/パッセージ)
...
2. **語彙サイクリング**: 限られた語彙で長文を実現
3. **段階的拡張**: 10段階の反復改善で目標達成
4. **自動化ツール**: Python スクリプトによる効率的生成

### 学んだ教訓

1. Introduction/Conclusionも語数に大きく影響
2. 接続表現・説明文の頻度が自然さと語数の両立のカギ
3. max_passesの増加は効果的だが、6回が実用的上限
4. セクション単位での語数調整が最も制御しやすい
