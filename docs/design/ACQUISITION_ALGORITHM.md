...
function getNextReviewQuestion(): QueueEntry | null {
  ...
  for (const entry of this.queues.immediate) {
    if (currentQ >= entry.targetQuestionNumber || 
        now >= entry.targetTime) {
...
function handleCorrectAnswer(
  ...
  if (currentQueue === QueueType.IMMEDIATE && 
      shouldEnqueueEarly(word, progress)) {
    enqueueToEarly(word);
  } else if (currentQueue === QueueType.EARLY && 
             shouldEnqueueMid(word, progress)) {
    enqueueToMid(word);
  } else if (currentQueue === QueueType.MID && 
             shouldEnqueueEnd(word, progress)) {
    enqueueToEnd(word);
  }
...
function isAcquisitionComplete(progress: AcquisitionProgress): boolean {
  ...
  if (recentReviews.length < 2 || 
      !recentReviews.every(r => r.isCorrect)) {
    return false;
  }
...
function enqueueWithDuplicateCheck(
  word: string,
  queueType: QueueType,
  entry: QueueEntry
): void {
