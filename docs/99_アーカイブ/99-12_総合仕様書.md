...
| [アーキテクチャガイド](docs/ARCHITECTURE_GUIDE.md) | **必読**: プロジェクト構造と設計思想 | 全員 |
...
A: 以下の7列固定のUTF-8エンコードのCSVファイルです：

```csv
語句,読み/発音,意味/和訳/史実名,語源/解説,関連語,関連分野,難易度
```

**重要**: 
- **列の順序は固定**です（ヘッダの名前は柔軟に変更可能）
- **列3（意味/和訳/史実名）** が選択肢として表示されます
- これにより、ヘッダの解釈に関わらず正確な選択肢が生成されます
