...
3. **長文タブ** - 長文読解と単語学習
4. **統計タブ** - 学習進捗の可視化と弱点分析
5. **設定タブ** - アプリケーション設定とデータ管理
6. **問題編集タブ** - 問題の閲覧と編集（管理用）
...
2. **カテゴリー/難易度フィルター** → `getFilteredQuestions()` → 出題問題
3. **適応的学習AI** → `selectAdaptiveQuestions()` → 弱点優先出題
4. **進捗記録** → `addQuizResult()` → LocalStorage保存
5. **統計分析** → `analyzeRadarChart()` → 弱点カテゴリー特定
