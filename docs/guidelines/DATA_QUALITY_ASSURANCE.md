...
2. **IPA_INVALID_CHARS**: IPA発音に無効な文字が含まれる
3. **MEANING_ENGLISH_ONLY**: 意味フィールドに英語のみが記述されている
4. **KATAKANA_ENGLISH_MIXED**: カタカナ発音に英語が混入
5. **MISSING_REQUIRED_FIELD**: 必須フィールドの欠損

### 優先度: 中（できるだけ早く修正）

1. **IPA_SAME_AS_WORD**: IPA発音が単語と同じ
2. **MEANING_NO_JAPANESE**: 意味フィールドに日本語が含まれない
3. **KATAKANA_INVALID_CHARS**: カタカナ発音に不適切な文字
