...
##### エラー1: 並列構造の読点欠落
英語の "A and B" を日本語に訳す際、読点が抜ける問題

❌ **誤り:**
```
英語: I like sports and music.
誤訳: スポーツと音楽が好きです。
```
...
英語: I like sports and music.
正訳: スポーツ、そして音楽が好きです。
     または: スポーツと音楽が好きです。（簡潔な場合は読点不要）
```

**判断基準:**
- 3つ以上の並列: **必ず読点** （例: A, B, and C → A、B、そしてC）
- 2つの並列（長い語句）: 読点推奨（例: 呼吸器疾患、そして心臓病）
- 2つの並列（短い語句）: 読点不要でも可（例: スポーツと音楽）

##### エラー2: カタカナ名詞の並列
英語の名詞列挙をカタカナで訳す際、読点が抜ける問題

❌ **誤り:**
```
英語: Sea turtles, dolphins, and whales
誤訳: ウミガメイルカクジラ
     または: ウミガメとイルカとクジラ
```
...
❌ **誤り:**
```
英語: When our teacher announced that our class would host an exchange student from Australia for two weeks, everyone felt excited.
誤訳: 教師が発表した時、私達のクラスはオーストラリアから交換学生をホストします、2週間、全員が興奮を感じました。
```

✅ **正しい:**
```
正訳: 先生が私たちのクラスでオーストラリアからの交換留学生を2週間受け入れると発表した時、みんな興奮しました。
```

**ポイント:**
- 日本語として自然な語順に並び替える
- 「私達」→「私たち」（ひらがな推奨）
- 「～をホストします」→「～を受け入れる」（自然な表現）
- 読点の位置を日本語として適切に配置

##### エラー4: 会話文の不自然な訳
会話文を直訳すると話し言葉として不自然になる問題

❌ **誤り:**
```
英語: "Good morning! It's time to get up."
誤訳: 「おはようございます!起きる時間です。」
```

✅ **正しい:**
```
正訳: 「おはよう!起きる時間よ」（親子の会話）
     または: 「おはようございます。起きる時間ですよ」（丁寧）
```

**ポイント:**
- 話者の関係性を考慮（親子、友人、先生-生徒など）
- 文末の口調を自然に（「です/ます」「だ/である」「よ/ね」など）
- 感嘆符（!）の後は日本語でも感嘆符または句点

**検証方法:**
```bash
# 過去の翻訳チェック結果を参照
cat docs/TRANSLATION_CHECK_REPORT.txt

# 並列構造の確認（「と」が連続していないか）
grep -E "と.{1,10}と" public/data/passages-translations/*.txt

# カタカナ名詞3つ以上で読点なし
grep -E "[ァ-ヶー]{2,}[ァ-ヶー]{2,}[ァ-ヶー]{2,}" public/data/passages-translations/*.txt
```
