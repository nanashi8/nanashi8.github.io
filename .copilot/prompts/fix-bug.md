...
**リファクタリング時に非推奨構文を使用していないか確認:**

### 1. 自動チェック
```bash
# 新しく作成したファイルをチェック
grep -r "\.onChange(of:.*) { _ in" [新しいファイル名].swift
```

### 2. コードレビュー
`.copilot/prompts/code-review.md` のチェックリストを実行：
...
### 3. ビルド確認
```bash
xcodebuild clean build | grep -i "deprecated"
```

**重要**: リファクタリングは既存コードの改善なので、非推奨構文があれば修正する絶好のチャンスです！
...
4. 非推奨構文を修正した場合は `.copilot/deprecated-patterns.md` を参照元として記録
```
